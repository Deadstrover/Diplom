...
from key_defs.key_codes import KeyCode
from macro.macro_action import SS_TAP_CODE, SS_DOWN_CODE, SS_UP_CODE, ActionText, ActionTap, ActionDown, ActionUp, \
    SS_QMK_PREFIX, SS_DELAY_CODE, ActionDelay, EXT_TAP, EXT_DOWN, EXT_UP
from macro.macro_action_ui import tag_to_action
from comm.base_comm import BaseProtocol
from comm.constants import CMD_MACRO_GET_COUNT, CMD_MACRO_GET_BUFFER_SIZE, CMD_MACRO_GET_BUFFER, \
    CMD_MACRO_SET_BUFFER, BUFFER_FETCH_CHUNK, PROTOCOL_ADV_MACROS
from security_dialog import Unlocker
        ...
        if data[0] == SS_QMK_PREFIX:
            ...
            if act in [SS_TAP_CODE, SS_DOWN_CODE, SS_UP_CODE,
                ...
                if act in [SS_TAP_CODE, SS_DOWN_CODE, SS_UP_CODE]:
                    ...
                    remap = {EXT_TAP: SS_TAP_CODE,
                             EXT_DOWN: SS_DOWN_CODE,
                             EXT_UP: SS_UP_CODE}
                    act = remap[act]
                    if len(data) < 4:
                        break
...
class ProtocolMacro(BaseProtocol):
    ...
    def reload_macros_early(self):
        ...
        data = self.usb_send(self.dev, struct.pack("B", CMD_MACRO_GET_COUNT), retries=20)
        self.macro_count = data[1]
        data = self.usb_send(self.dev, struct.pack("B", CMD_MACRO_GET_BUFFER_SIZE), retries=20)
    ...
    def reload_macros_late(self):
        ...
        if self.macro_memory:
            ...
            for x in range(0, self.macro_memory, BUFFER_FETCH_CHUNK):
                ...
                data = self.usb_send(self.dev, struct.pack(">BHB", CMD_MACRO_GET_BUFFER, x, sz), retries=20)
    ...
    def set_macro(self, data):
        ...
        for x, chunk in enumerate(chunks(data, BUFFER_FETCH_CHUNK)):
            ...
            self.usb_send(self.dev, struct.pack(">BHB", CMD_MACRO_SET_BUFFER, off, len(chunk)) + chunk,
                          retries=20)
        self.macro = data
    ...
    def macro_serialize(self, macro):
        ...
        out = b""
        for action in macro:
            out += action.serialize(self.comm_protocol)
        return out
    ...
    def macro_deserialize(self, data):
        ...
        """
        if self.comm_protocol >= PROTOCOL_ADV_MACROS:
